...
app.get("/employee/:employeeId/name", (req, res) => {
  const { employeeId } = req.params;

  const query = "SELECT full_name AS name FROM employees WHERE id = ?";
...
app.delete('/api/certificates/:id', authenticateToken, async (req, res) => {
  try {
      ...
      const result = await db.query('DELETE FROM certificates WHERE id = ?', [certId]);
